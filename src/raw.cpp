...
int moveSteps_x1 = x;   
int moveSteps_x2 = x+80; 
int moveSteps_y1 = y; 
int moveSteps_y2 = y+80; 
// ================= 4. 初始化 =================
void setup() {
    Serial.begin(115200);
...
void readSerialXY() {
    ...
    while (Serial.available()) {
        ...
        if (c == '\n') {
            ...
            if (input.startsWith("[") && input.endsWith("]") && commaIndex > 0) {
                String sx = input.substring(1, commaIndex);
                String sy = input.substring(commaIndex + 1, input.length() - 1);
